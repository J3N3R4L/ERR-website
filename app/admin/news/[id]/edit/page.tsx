...
export default async function NewsEditPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Edit News Post" subtitle="Update and publish content." />

      <Card>
        <CardContent>
          <form method="post" action={`/api/admin/news/${typedPost.id}`} className="grid gap-4 md:grid-cols-2">
            <label className="block text-sm">
              Title (EN)
              <Input name="title_en" required defaultValue={typedPost.title_en} className="mt-1" />
            </label>
            <label className="block text-sm">
              Title (AR)
              <Input name="title_ar" required defaultValue={typedPost.title_ar} className="mt-1" />
            </label>
            <label className="block text-sm">
              Slug
              <Input name="slug" required defaultValue={typedPost.slug} className="mt-1" />
            </label>
            {canSelectAnyLocality ? (
              <label className="block text-sm">
                Locality (optional)
                <Select name="locality_id" className="mt-1" defaultValue={typedPost.locality_id ?? ""}>
            ...
            <label className="block text-sm">
              Status
              <Select name="status" className="mt-1" defaultValue={typedPost.status}>
